...
# usermods/CloudLink, platformio_override.ini), which a merge cannot touch. A few core files
# ARE edited for branding, the cloud pairing UI and two WiFi fixes; if an upstream change ever
# rewrites them, git may merge cleanly while dropping our block. This script catches that.
#
# Run after every upstream merge (this branch tracks upstream release tags, v16.0.1 first).
